...
RESET_CAUSES = {}
for _n in ("PWRON_RESET", "HARD_RESET", "WDT_RESET", "DEEPSLEEP_RESET",
...
WAKE_REASONS = {}
for _n in ("PIN_WAKE", "EXT0_WAKE", "EXT1_WAKE", "TIMER_WAKE",
           "TOUCHPAD_WAKE", "ULP_WAKE"):
    _c = getattr(machine, _n, None)
    if _c is not None:
        WAKE_REASONS[_c] = _n
...
class SleepDiagnostics:
    ...
    def info(self, show=True):
        ...
        info = {
            ...
            "wake_reason": wake,
            "wake_reason_name": (WAKE_REASONS.get(wake, "code({})".format(wake))
                                 if wake is not None else "n/a"),
            "rtc_marker": mem.startswith(_MARK),
        }
        if show:
            print("  Reset cause: {} ({})".format(
                info["reset_cause_name"], cause))
            print("  Wake reason: {}".format(info["wake_reason_name"]))
            print("  RTC marker : {}".format(
                "present (deep-sleep test pending check)" if info["rtc_marker"]
                else "none"))
        return info
    ...
    def light_sleep(self, ms=2000, show=True):
        ...
        if show:
            print("  Resumed. Measured elapsed: {} ms (requested {} ms)".format(
                dt, ms))
    ...
    def deep_sleep(self, seconds=5, show=True):
        ...
        try:
            ...
            print("  warning: RTC memory unavailable ({}); wake won't self-report".format(e))
        if show:
            print("  Deep sleep {} s. The board WILL REBOOT and the serial/REPL".format(
                seconds))
            print("  link will drop. Reconnect after ~{} s; the wake result".format(
                seconds))
    ...
    def check_wake(self, show=True, clear=True):
        ...
        if show:
            ...
            print("  Result     : {}".format(
                "PASS (woke via deep-sleep reset)" if ok
                else "check: reset cause was not DEEPSLEEP_RESET"))
            print("=" * 78)
        return {"woke_from_test": True, "reset_cause": cause,
